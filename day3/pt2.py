...
def translateCoordinates(wire):
    ...
    for command in wire:
        if command[0] == 'R':
            xCoord += int(command[1:])
            visited.append((xCoord, yCoord))
        elif command[0] == 'L':
            xCoord -= int(command[1:])
            visited.append((xCoord, yCoord))
        elif command[0] == 'U':
            yCoord += int(command[1:])
            visited.append((xCoord, yCoord))
        elif command[0] == 'D':
            yCoord -= int(command[1:])
            visited.append((xCoord, yCoord))
...
def checkFirstCollision(path1, path2): # This may actually be the absolute slowest way to do this, don't try this at home (or work)
    collisions = []
    for coords in path1:
        if coords in path2:
...
coords1 = translateCoordinates(wire1)
coords2 = translateCoordinates(wire2)

path1 = tracePath(coords1)
path2 = tracePath(coords2)
path1.remove((0, 0))
path2.remove((0, 0))
collisions = checkFirstCollision(path1, path2)

shortest = 999999999999999
for collision in collisions:
    shortest = path1.index(collision) + path2.index(collision) if path1.index(collision) + path2.index(collision) < shortest else shortest

print(shortest + 2) # because reasons
